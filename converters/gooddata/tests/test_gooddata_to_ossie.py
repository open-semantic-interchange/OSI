...
from ossie_gooddata.gooddata_to_ossie import (
    _convert_attribute,
    _convert_fact,
    gooddata_to_ossie,
)
...
def test_native_source_types_become_ossie_datatypes(gooddata_type: str, ossie_type: str):
    ...
    ossie_attribute = _convert_attribute(attribute, "orders")
    ossie_fact = _convert_fact(fact, "orders")

    assert ossie_attribute["datatype"] == ossie_type
    assert ossie_attribute["dimension"]["is_time"] is False
    assert ossie_fact["datatype"] == ossie_type
    assert "dimension" not in ossie_fact
...
def test_unknown_source_type_becomes_opaque_and_is_preserved(source_type: str, converter, model):
    ...
    ossie_field = converter(model, "orders")

    assert ossie_field["datatype"] == "Opaque"
    extension = json.loads(ossie_field["custom_extensions"][0]["data"])
