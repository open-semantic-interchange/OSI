...
def gooddata_to_ossie(
    ...
    for ds in model.ldm.datasets:
        ossie_ds, rels = _convert_dataset(ds, attr_source_col_map)
        ossie_datasets.append(ossie_ds)
        ossie_relationships.extend(rels)

    # Convert date instances to datasets
    for di in model.ldm.date_instances:
        ossie_datasets.append(_convert_date_instance(di))
    ...
    if ossie_relationships:
        semantic_model["relationships"] = ossie_relationships
...
def _convert_dataset(
    ...
    ossie_ds: dict[str, Any] = {
        "name": ds.id,
        "source": source,
    }
    if primary_key:
        ossie_ds["primary_key"] = primary_key
    if ds.description:
        ossie_ds["description"] = ds.description
    if ds.title != ds.id:
...
def _convert_date_instance(di: GdDateInstance) -> dict[str, Any]:
    ...
    if di.description:
        ossie_ds["description"] = di.description
    if di.title != di.id:
        ossie_ds["ai_context"] = {"synonyms": [di.title]}
...
def _convert_attribute(attr: GdAttribute, dataset_id: str) -> dict[str, Any]:
    ...
    if datatype is not None:
        ossie_field["datatype"] = datatype
    if attr.description:
        ossie_field["description"] = attr.description
    if attr.title != attr.source_column:
        ossie_field.setdefault("ai_context", {})["synonyms"] = [attr.title]
...
def _convert_fact(fact: GdFact, dataset_id: str) -> dict[str, Any]:
    ...
    if datatype is not None:
        ossie_field["datatype"] = datatype
    if fact.description:
        ossie_field["description"] = fact.description
    if fact.title != fact.source_column:
        ossie_field.setdefault("ai_context", {})["synonyms"] = [fact.title]
    if datatype == "Opaque":
        ossie_field["custom_extensions"] = [
            {
                "vendor_name": "GOODDATA",
