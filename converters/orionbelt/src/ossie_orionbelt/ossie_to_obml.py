...
"""Ossie → OBML conversion (the :class:`OssietoOBML` direction).
...
class OssietoOBML:
    """Convert an Ossie semantic model YAML to OBML format."""

    def __init__(
        self, ossie: dict, default_database: str = "ANALYTICS", default_schema: str = "PUBLIC"
    ):
        self.ossie = ossie
        self.default_database = default_database
        self.default_schema = default_schema
        self.warnings: list[str] = []
        # Ossie metrics that have no OBML representation (non-SQL dialect only,
    ...
    def _normalize_legacy_v01(self) -> None:
        ...
        """
        version = str(self.ossie.get("version", ""))
        ...
        if version.startswith(("0.0", "0.1")):
            self.warnings.append(
                f"Ossie input declares version '{version}'; legacy v0.1.x "
                f"compatibility shim applied. Output target is v{_OSSIE_VERSION}."
            )
    ...
    def convert(self) -> dict:
        ...
        # ── Dimensions (extracted from Ossie fields with dimension metadata) ──
        ...
        # original Ossie metric verbatim under the Ossie vendor so the reverse
        # (OBML -> Ossie) direction re-emits them and a full Ossie -> OBML -> Ossie
    ...
    def _carry_foreign_extensions(ossie_exts: list[dict] | None, obml_target: dict[str, Any]) -> None:
        ...
        Our own payloads and Ossie-native stashes are reconstructed elsewhere;
        any other vendor's extension is preserved unchanged on the OBML side
        so a full Ossie -> OBML -> Ossie roundtrip keeps the original vendor.
        """
        for ext in ossie_exts or []:
    ...
    def _convert_dataset(self, ds: dict, rel_by_from: dict) -> tuple[str, dict]:
        """Convert an Ossie dataset to an OBML dataObject.

        Uses the exact Ossie dataset name as the OBML data object key.
        """
        name = ds["name"]
        ...
        # ── Primary key flag propagation (Ossie v0.2 first-class) ──
        ...
        # ── Description (semantic, from Ossie) ─────────────────────────
        ...
        # Persist the Ossie ``unique_keys`` array into the OBSL-vendor
        # customExtensions so the OBML → Ossie direction can emit it back.
    ...
    def _convert_field(self, field: dict) -> tuple[str, dict]:
        """Convert an Ossie field to an OBML column.

        Uses the exact Ossie field name as the OBML column key.
        """
        name = field["name"]
        ...
        # ── Field label roundtrip (Ossie v0.2 first-class) ──
    ...
    def _infer_obml_type(self, field: dict) -> str:
        """Infer OBML abstractType from Ossie field metadata."""
    ...
    # Ossie relationship type → OBML joinType mapping
    ...
    def _convert_relationship_to_join(self, rel: dict) -> dict:
        """Convert an Ossie relationship to an OBML join.

        Uses exact Ossie names for joinTo and column references.
        Maps Ossie relationship 'type' to OBML joinType if present,
    ...
    def _extract_dimensions(self, datasets: list) -> dict:
        """Extract dimension definitions from Ossie fields marked as dimensions.
    ...
    def _convert_metrics(self, ossie_metrics: list, ds_map: dict) -> tuple[dict, dict]:
        """
        Convert Ossie metrics to OBML measures and metrics.

        Ossie has a single 'metrics' concept with SQL expressions.
        ...
        # back to their canonical Ossie names (Snowflake/Databricks expressions
        # commonly upper-case or quote them). Identifiers resolve by BOTH the
        # Ossie name and the physical code (source-table code / bare-identifier
        # field expression): our own OBML -> Ossie emitter writes metric SQL
        ...
        for ds_name, ds in ds_map.items():
            fmap: dict[str, str] = {}
            ossie_fields = ds.get("fields", []) or []
            for f in ossie_fields:
        ...
        for m in ossie_metrics:
            name = m["name"]

            ossie_description = m.get("description")

            # Extract synonyms from Ossie ai_context
            ossie_ai_ctx = m.get("ai_context")
            ossie_synonyms: list[str] = []
            if isinstance(ossie_ai_ctx, dict) and ossie_ai_ctx.get("synonyms"):
                ossie_synonyms = list(ossie_ai_ctx["synonyms"])
            ...
            if obml_extras.get("obml_metric_type") == "cumulative":
                cum_metric = self._reconstruct_cumulative_metric(
                    name, obml_extras, ossie_description, ossie_synonyms
                )
            ...
            # marker comes from the OBML → Ossie direction; on input we restore
            # ``aggregation: measure`` without touching the Ossie expression
            ...
            if obml_extras.get("obml_aggregation") == "measure":
                ...
                if ossie_description:
                    delegated["description"] = ossie_description
                if ossie_synonyms:
                    delegated["synonyms"] = ossie_synonyms
            ...
            # Resolve `dataset.column` references to canonical Ossie names
            ...
            if parsed:
                ...
                if ossie_description:
                    measure_def["description"] = ossie_description
                if ossie_synonyms:
                    measure_def["synonyms"] = ossie_synonyms
            ...
            if auto_measures:
                ...
                if ossie_description:
                    metric_def["description"] = ossie_description
                if ossie_synonyms:
                    metric_def["synonyms"] = ossie_synonyms
                ...
                # the metric survives an Ossie -> OBML -> Ossie roundtrip.
                self._preserve_unconverted_metric(
        ...
        # OBML entity (measure or metric) the Ossie metric became.
        for m in ossie_metrics:
    ...
    @staticmethod
    def _extract_obml_extras(ossie_metric: dict) -> dict:
        """Extract OBML-only properties from Ossie metric custom_extensions."""
        for ext in ossie_metric.get("custom_extensions", []):
            if ext.get("vendor_name") in _OBML_VENDOR_READ:
                try:
        ...
        for dialect in expr.get("dialects", []) or []:
            if isinstance(dialect, dict):
                ...
                if isinstance(text, str):
                    candidate = OssietoOBML._unquote_identifier(text)
    ...
    def _resolve_column_refs(
        ...
        """Rewrite ``dataset.column`` references to canonical Ossie names.
    ...
    def _preserve_unconverted_metric(self, ossie_metric: dict, reason: str) -> None:
        """Preserve an Ossie metric that has no OBML representation.

        OBML cannot express the metric, but dropping it silently would break
        the README's roundtrip promise. Instead the original Ossie metric is kept
        verbatim (re-emitted on OBML -> Ossie) and a loud LOSSY warning is raised:
        the metric is preserved but NOT queryable through OBML.
        """
        self._unconverted_metrics.append(ossie_metric)
        self.warnings.append(
            f"LOSSY: Ossie metric '{ossie_metric.get('name', '?')}' has no OBML representation "
            f"({reason}); preserved verbatim for Ossie -> OBML -> Ossie roundtrip but it is "
    ...
    def _decompose_complex_metric(self, name: str, expr: str) -> tuple[str, dict]:
        """
        Decompose a complex Ossie metric expression (multiple aggregations)
