...
"""Third-party Ossie metrics must never be silently lost on import.

Regression coverage for the review on Ossie PR #153: the converter only emits
``ANSI_SQL`` on the OBML -> Ossie path, so our own roundtrip corpus never exercised
the import drop paths. A genuinely foreign Ossie model (Snowflake/Databricks
...
def _ossie_model(metrics: list[dict[str, Any]]) -> dict[str, Any]:
...
class TestDialectCatching:
    ...
    def test_snowflake_simple_agg_becomes_measure(self) -> None:
        ossie = _ossie_model([_metric("Total Amount", "SNOWFLAKE", "SUM(Orders.amount)")])
    ...
    def test_databricks_expression_agg_becomes_measure(self) -> None:
        ossie = _ossie_model([_metric("Net", "DATABRICKS", "SUM(Orders.amount * Orders.amount)")])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
    ...
    def test_snowflake_uppercased_identifiers_resolve_to_canonical(self) -> None:
        ...
        # the real Ossie dataset/field names, not produce refs to ORDERS.AMOUNT.
        ossie = _ossie_model([_metric("Total Amount", "SNOWFLAKE", "SUM(ORDERS.AMOUNT)")])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
    ...
    def test_decimal_literals_not_rewritten_as_refs(self) -> None:
        ...
        ossie = _ossie_model([_metric("Scaled", "ANSI_SQL", "SUM(Orders.amount * 1.23)")])
    ...
    def test_expression_agg_identifiers_resolved(self) -> None:
        ...
        ossie = _ossie_model([_metric("Net", "SNOWFLAKE", "SUM(ORDERS.AMOUNT * ORDERS.ID)")])
        obml = conv.OssietoOBML(ossie).convert()
    ...
    def test_unmappable_reference_is_preserved_not_dangling(self) -> None:
        ...
        ossie = _ossie_model([_metric("Bogus", "SNOWFLAKE", "SUM(Orders.nonexistent)")])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
    ...
    def test_unknown_dataset_reference_is_preserved(self) -> None:
        ossie = _ossie_model([_metric("Cross", "SNOWFLAKE", "SUM(Unknown.amount)")])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
...
class TestNoSilentLoss:
    ...
    def test_non_sql_dialect_is_preserved_not_dropped(self) -> None:
        ossie = _ossie_model([_metric("Mdx Thing", "MDX", "[Measures].[Amount]")])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
        ...
        # ...but preserved verbatim under the Ossie vendor...
    ...
    def test_real_maql_metric_syntax_is_preserved(self) -> None:
        ...
        ossie = _ossie_model([_metric("Total Profit", "MAQL", maql)])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
    ...
    def test_undecomposable_sql_is_preserved_not_dropped(self) -> None:
        ...
        ossie = _ossie_model([_metric("Ratio", "ANSI_SQL", "Orders.amount / Orders.id")])
        converter = conv.OssietoOBML(ossie)
        obml = converter.convert()
...
class TestRestoredMetricMetadata:
    ...
    a (non-conformant) root array. See Ossie PR #148."""

    def test_preserved_metric_keeps_dialect_and_vendor_on_the_metric(self) -> None:
        # MDX metric carrying a GOODDATA custom extension. After Ossie -> OBML ->
        # Ossie the metric round-trips with its dialect on expression.dialects[]
    ...
    def test_no_root_dialects_or_vendors_emitted(self) -> None:
        ...
        obml = conv.OssietoOBML(ossie).convert()
        ossie_again = conv.OBMLtoOssie(obml, "sales").convert()
        assert "dialects" not in ossie_again
        assert "vendors" not in ossie_again
...
class TestStaleStashNameCollision:
    ...
    def test_collision_drops_preserved_copy_not_duplicate(self) -> None:
        ...
        ossie = _ossie_model([_metric("Revenue", "MDX", "[Measures].[Rev]")])
        obml = conv.OssietoOBML(ossie).convert()
    ...
    def test_output_passes_ossie_validation_after_collision(self) -> None:
        pytest.importorskip("jsonschema")  # validate_ossie needs it
        ossie = _ossie_model([_metric("Revenue", "MDX", "[Measures].[Rev]")])
        obml = conv.OssietoOBML(ossie).convert()
        obml["measures"] = {
            "Revenue": {
        ...
        ossie_again = conv.OBMLtoOssie(obml, "sales").convert()
        result = conv.validate_ossie(ossie_again)
...
class TestIdempotency:
    ...
    def test_ossie_to_obml_convert_is_idempotent(self) -> None:
        ossie = _ossie_model(
            [
    ...
    def test_obml_to_ossie_convert_is_idempotent(self) -> None:
        obml = conv.OssietoOBML(
            _ossie_model([_metric("Mdx Thing", "MDX", "[Measures].[Amount]")])
        ).convert()
        converter = conv.OBMLtoOssie(obml, "sales")
        converter.convert()
...
def _unconverted_stash(obml: dict[str, Any]) -> list[dict[str, Any]]:
    """Pull the preserved Ossie metrics out of the Ossie-vendor customExtension."""
    import json

    for ext in obml.get("customExtensions", []) or []:
        if ext.get("vendor") in ("Ossie", "OBSL"):
