...
"""Ossie v0.2 compatibility tests.
...
def schema_validator() -> Any:
    ...
    with open(conv._OSSIE_SCHEMA_PATH) as f:
        schema = json.load(f)
    return jsonschema.Draft202012Validator(schema)


def _collect_vendor_names(ossie: dict[str, Any]) -> set[str]:
    """Every ``custom_extensions[].vendor_name`` anywhere in an Ossie document."""
    found: set[str] = set()
...
class TestEmittedVersion:
    def test_top_level_version_is_v02(self) -> None:
        ...
        assert ossie["version"] == conv._OSSIE_VERSION
    ...
    def test_vendor_tagged_per_entity_not_root(self) -> None:
        ...
        # not a root array. The OBML field label round-trips as a *native* Ossie
...
class TestPrimaryKey:
    ...
    def test_unknown_pk_column_emits_warning(self) -> None:
        ...
        converter = conv.OssietoOBML(bad)
        converter.convert()
...
class TestLegacyShim:
    def test_v01_input_promotes_primary_key(self) -> None:
        # The shim mutates self.ossie in place — capture the dataset shape
        # before parsing finishes by snapshotting after _normalize_legacy_v01.
        converter = conv.OssietoOBML(_OSSIE_V01_INPUT.copy())
    ...
    def test_v01_input_warns(self) -> None:
        converter = conv.OssietoOBML(json.loads(json.dumps(_OSSIE_V01_INPUT)))
        converter.convert()
        joined = " ".join(converter.warnings)
    ...
    def test_v02_input_not_normalized(self) -> None:
        ...
        converter = conv.OssietoOBML(v02)
        converter.convert()
...
class TestSchemaValidation:
    ...
    def test_v01_via_shim_revalidates_clean(self, schema_validator: Any) -> None:
        obml = conv.OssietoOBML(json.loads(json.dumps(_OSSIE_V01_INPUT))).convert()
        ossie = conv.OBMLtoOssie(obml).convert()
        errors = list(schema_validator.iter_errors(ossie))
        assert errors == [], [e.message for e in errors[:5]]

    def test_tpcds_fixture_passes_v02_validation(self, schema_validator: Any) -> None:
        """Real-world TPC-DS OBML model must emit v0.2-clean Ossie."""
