...
"""Regression tests for issue #201: Ossie<->OBML converter round-trip fidelity
and validation robustness.

Covers three bugs found during the apache/ossie#153 review:

- P1: metric references broke the OBML -> Ossie -> OBML round trip when a data
...
- P2: dimensions extracted from Ossie collided when the same field name occurred
  in more than one dataset (the second silently overwrote the first).
- P2: ``validate_ossie`` raised ``AttributeError`` on malformed input instead of
...
class TestMetricRoundTripCodeVsName:
    ...
    def test_third_party_field_name_differs_from_code(self) -> None:
        # Snowflake-style Ossie: field display name != physical column code, and
    ...
    def test_physical_code_maps_to_field_name_with_space(self) -> None:
        # Ossie field display name contains a space; the metric references the
...
# ---------------------------------------------------------------------------
# P2: validate_ossie robustness on malformed input
# ---------------------------------------------------------------------------


class TestValidateOssieRobustness:
