...
"""Tests for Ossie ↔ OBML cumulative metric conversion.

Validates that cumulative metrics survive the OBML → Ossie → OBML roundtrip
...
class TestOBMLtoOssieCumulative:
    """OBML → Ossie: cumulative metrics are serialized into custom_extensions."""
    ...
    def test_running_total_exported(self) -> None:
        ossie, warnings = self._convert()
        m = self._find_metric(ossie, "Running Revenue")
    ...
    def test_rolling_window_exported(self) -> None:
        ossie, _ = self._convert()
        m = self._find_metric(ossie, "Rolling 7d Revenue")
        assert m is not None
    ...
    def test_grain_to_date_exported(self) -> None:
        ossie, _ = self._convert()
        m = self._find_metric(ossie, "MTD Revenue")
        assert m is not None
    ...
    def test_derived_metric_still_exported(self) -> None:
        ...
        ossie, _ = self._convert()
        m = self._find_metric(ossie, "Derived Metric")
        assert m is not None
        expr = m["expression"]["dialects"][0]["expression"]
...
class TestOssietoOBMLCumulative:
    """Ossie → OBML: cumulative metrics are reconstructed from custom_extensions."""

    def _roundtrip_ossie(self) -> dict:
        """OBML → Ossie → OBML roundtrip."""
        converter1 = conv.OBMLtoOssie(_OBML_WITH_CUMULATIVES)
...
class TestCumulativeEdgeCases:
    ...
    def test_cumulative_with_synonyms_roundtrip(self) -> None:
        ...
        converter1 = conv.OBMLtoOssie(obml)
        ossie = converter1.convert()
    ...
    def test_mixed_model_all_metric_types(self) -> None:
        ...
        assert "Revenue" in metric_names  # measure → Ossie metric
