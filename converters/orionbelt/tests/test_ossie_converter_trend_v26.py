...
"""Ossie ↔ OBML roundtrip for v2.6 trend-analysis fields.

Validates that:
- ``partitionBy`` survives OBML → Ossie → OBML on cumulative metrics
...
def _extras(ossie_metric: dict) -> dict:
...
class TestObmlToOssieV26:
    def setup_method(self) -> None:
        self.ossie = conv.OBMLtoOssie(_OBML_V26).convert()

    def test_partition_by_preserved_on_cumulative(self) -> None:
        m = _ossie_metric(self.ossie, "Revenue MA3 by Country")
        extras = _extras(m)
    ...
    def test_window_dense_rank_preserved(self) -> None:
        m = _ossie_metric(self.ossie, "Revenue Rank")
        extras = _extras(m)
    ...
    def test_window_lag_preserved(self) -> None:
        m = _ossie_metric(self.ossie, "Revenue Prior Month")
        extras = _extras(m)
    ...
    def test_window_ntile_preserved(self) -> None:
        m = _ossie_metric(self.ossie, "Revenue Quartile")
        extras = _extras(m)
    ...
    def test_corr_measure_emits_two_columns(self) -> None:
        # Find Revenue Spend Corr in Ossie metrics
        m = _ossie_metric(self.ossie, "Revenue Spend Corr")
    ...
    def test_derived_metric_referencing_multi_col_measure_emits_all_columns(
        ...
        to Ossie SQL. Before the fix, ``_measure_to_sql`` only emitted
        ``columns[0]`` so the derived expression carried garbage
        ``CORR(orders.amount)``.
        """
        m = _ossie_metric(self.ossie, "Corr Doubled")
        expr = m["expression"]["dialects"][0]["expression"]
...
class TestOssieToObmlV26:
    def setup_method(self) -> None:
        self.ossie = conv.OBMLtoOssie(_OBML_V26).convert()
        self.obml = conv.OssietoOBML(self.ossie).convert()
