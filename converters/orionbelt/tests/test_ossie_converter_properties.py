...
"""Tests for Ossie ↔ OBML roundtrip of data types, settings, owner, and column properties.

Validates that recently added OBML properties survive the OBML → Ossie → OBML
...
def _roundtrip(obml: dict) -> dict:
    ...
    fwd = conv.OBMLtoOssie(obml)
    ossie = fwd.convert()
    back = conv.OssietoOBML(ossie)
    return back.convert()
...
class TestRefreshPolicy:
    """OBML refresh: block round-trips through Ossie custom_extensions."""
...
class TestColumnProperties:
    ...
    Note: Ossie uses column *code* as field name, so after roundtrip the OBML
...
class TestDimensionProperties:
    ...
    Note: Ossie uses column *code* as the field name, so after roundtrip the
...
class TestCountSynthesisKnobs:
    ...
    load), so they are never emitted to Ossie; only the knobs must survive.
    """
    ...
    def test_no_synthesized_count_measures_emitted(self):
        ossie = conv.OBMLtoOssie(self._OBML_COUNTS).convert()
        # No ``.count`` measure leaks into the Ossie datasets/metrics.
        blob = str(ossie)
