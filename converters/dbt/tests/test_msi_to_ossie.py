...
def _field_expr(result: OssieDocument, field_idx: int = 0) -> str:
...
class TestBasicConversion:
    ...
    def test_serialized_output_omits_schema_invalid_root_advertisement_fields(self) -> None:
        ...
        data = yaml.safe_load(result.to_ossie_yaml())
...
class TestRelationshipConversion:
    ...
    def test_three_datasets_produce_all_pairs(self, snapshot: SnapshotAssertion) -> None:
        ...
        assert result.to_ossie_yaml() == snapshot
...
class TestMetricConversion:
    ...
    def test_simple_metric_description_carried_over(self) -> None:
        ...
        assert _ossie_metrics(result)[0].description == "Total revenue"
    ...
    def test_ratio_metric_inlines_sub_expressions(self, snapshot: SnapshotAssertion) -> None:
        ...
        arpu_ossie = next(m for m in _ossie_metrics(result) if m.name == "arpu")
        assert arpu_ossie.expression.dialects[0].expression == (
    ...
    def test_derived_metric_inlines_sub_expressions(self) -> None:
        ...
        result = (
            MSIToOssieConverter().convert(_manifest(semantic_models=[sm], metrics=[revenue_m, cost_m, profit])).output
        )
    ...
    def test_derived_metric_nested(self, snapshot: SnapshotAssertion) -> None:
        ...
        net_ossie = next(m for m in _ossie_metrics(result) if m.name == "net_profit")
        assert (
            net_ossie.expression.dialects[0].expression
    ...
    def test_derived_metric_ref_not_corrupted_by_prefix_match(self) -> None:
        ...
        derived_ossie = next(m for m in _ossie_metrics(result) if m.name == "revenue_delta")
        assert derived_ossie.expression.dialects[0].expression == "SUM(orders.amount) - SUM(orders.adjusted_amount)"
    ...
    def test_cumulative_metric_uses_base_aggregation(self) -> None:
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm], metrics=[cumulative])).output
    ...
    def test_cumulative_metric_via_sub_metric_reference(self) -> None:
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm], metrics=[base, cumulative])).output

        cumulative_ossie = next(m for m in _ossie_metrics(result) if m.name == "cumulative_revenue")
        assert cumulative_ossie.expression.dialects[0].expression == "SUM(orders.amount)"

    # --- Edge cases ---

    def test_no_metrics_produces_no_ossie_metrics(self) -> None:
        sm = semantic_model_with_guaranteed_meta(name="orders")
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm])).output
...
class TestConverterIssues:
    def test_conversion_metric_emits_issue(self) -> None:
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm], metrics=[conversion]))
    ...
    def test_private_metric_emits_issue(self) -> None:
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm], metrics=[private_metric]))
    ...
    def test_natural_entity_emits_issue(self) -> None:
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm]))
    ...
    def test_cumulative_metric_emits_issue(self) -> None:
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm], metrics=[base, cumulative]))
...
class TestMetricFilterFlattening:
    def test_metric_level_filter_inlines_case_when(self) -> None:
        ...
        assert (
            _ossie_metrics(result)[0].expression.dialects[0].expression
    ...
    def test_metric_and_measure_filters_combined_with_and(self, snapshot: SnapshotAssertion) -> None:
        ...
        assert _ossie_metrics(result)[0].expression.dialects[0].expression == (
    ...
    def test_ratio_metric_filter_propagated_to_both_sides(self) -> None:
        ...
        paid_arpu = next(m for m in _ossie_metrics(result) if m.name == "paid_arpu")
    ...
    def test_derived_metric_filter_propagated_to_sub_expressions(self) -> None:
        ...
        paid_profit = next(m for m in _ossie_metrics(result) if m.name == "paid_profit")
    ...
    def test_no_filter_produces_plain_expression(self) -> None:
        ...
        assert _ossie_metrics(result)[0].expression.dialects[0].expression == "SUM(orders.amount)"


class TestOssieJsonSerialization:
    def test_to_ossie_json_produces_valid_json(self) -> None:
        sm = semantic_model_with_guaranteed_meta(
            name="orders",
            description="Order data",
            dimensions=[_dimension("ds", dim_type=DimensionType.TIME, granularity=TimeGranularity.DAY)],
        ...
        result = MSIToOssieConverter().convert(_manifest(semantic_models=[sm]), ossie_model_name="my_project").output
