...
"""Convert Omni semantic model files to an Ossie semantic model.
...
`views/*.view.yaml`, `topics/*.topic.yaml`. Omni features Ossie has no native field
...
Usage (CLI):
    ossie-omni import -i omni_model/ [-o model.yaml] [--name NAME] [--topic TOPIC]
"""
...
# aggregate_type values whose Ossie expression the exporter can rebuild exactly.
...
# measure is stashed verbatim, so the Omni -> Ossie -> Omni trip stays lossless;
# the expression is what other Ossie consumers see.
...
def convert_omni_to_ossie(files, model_name=None, topic=None):
    """Convert Omni model files ({relative filename: YAML str}) to Ossie YAML.

    `model_name` overrides the Ossie model name (default: the mapped topic's name,
    else 'omni_model'). `topic` names the topic whose description/AI context map
    onto the Ossie model when the directory holds more than one topic.
    """
    ...
    topic_paths = {}
    unmapped_views = set()  # view names present as files but with no Ossie dataset
    model_yaml = None
    rel_entries = []
    extra_files = {}
    for fname, text in files.items():
        ...
        if qv:
            # Query views are backed by a saved query, not a table; Ossie has no
            # dataset form for them. Preserved verbatim, restored on export.
            _warn(f"file '{fname}'", "query views have no Ossie dataset form; "
        ...
        if mv:
            ...
            if source is None:
                ...
                # has no standalone Ossie dataset form. Preserved verbatim.
                _warn(f"view '{vname}'",
    ...
    # The mapped topic supplies the Ossie model's name/description/ai_context.
    mapped_name = None
    ...
    # A join Ossie cannot represent -- one touching a view with no Ossie dataset (a
    ...
    for i, entry in enumerate(rel_entries):
        ...
        if endpoints & unmapped_views:
            _warn(f"relationship #{i + 1}",
                  "references a view with no Ossie dataset form; preserved in "
                  "custom_extensions only")
        else:
        ...
        if rel is None:
            ...
            continue
        # Ossie relationship names are unique per model; several (aliased) joins
...
def _convert_view(vname, view, meta):
    ...
    for c in raw_compound:
        translated, _ = omni_sql_to_ossie(str(c), vname)
    ...
    # no SQL (or Ossie) form at all; such dimensions/measures are stashed whole
    # and dropped from the Ossie model. Popped here so the later measure pass
        ...
        if templated:
            for n in templated:
                _warn(f"{kind[:-1]} '{vname}.{n}'",
                      ...
                      "Ossie form; preserved in custom_extensions only")
                entries.pop(n)
            stash[key] = templated
    ...
    # rebuild it from the Ossie primary_key alone (a `${view.field}` reference
    ...
    for key in ("description", "ai_context"):
        # Present-but-empty metadata has no Ossie slot; preserve it as an extra.
...
def _convert_dimension(vname, dname, dim):
    """Build one Ossie field from an Omni dimension. Returns (field, column) where
    ...
    if sql is None:
        ...
        sql = str(sql)
        expr, changed = omni_sql_to_ossie(sql, vname)
        if has_timeframe_ref(sql):
            _warn(scope, "timeframe reference (${view.field[timeframe]}) has no Ossie "
                         "form; flattened to the base field, original sql stashed")
        if changed or sql.strip() == dname:
            # Stashed when the Ossie expression differs from the Omni sql, and
    ...
    for key, value in dim.items():
        ...
        if key in _DIM_NATIVE_KEYS:
            # A present-but-empty native value (description: '') has no Ossie
            # slot -- Ossie omits empty metadata -- so it rides in the stash.
...
def _convert_relationship(entry, index, views):
    ...
    for clause in re.split(r"\s+AND\s+", on_sql, flags=re.IGNORECASE):
        ...
        if not m:
            # A valid Omni join Ossie cannot express (a range/non-equi or cross
            ...
            _warn(what, f"('{from_view}' -> '{to_view}'): on_sql clause "
                        ...
                        f"${{view.field}} references, so it has no Ossie "
    ...
    stash = {}
    # Export rebuilds on_sql from the Ossie columns in canonical form; when that
    ...
    if rel_type == REL_ONE_TO_MANY:
        # Ossie `from` is always the many side; flip the orientation (the stashed
    ...
    if entry.get("where_sql"):
        ...
        _warn(what, "where_sql (join filter) has no Ossie form; preserved in "
                    "custom_extensions only")
...
def _convert_measures(views, relationships, base_view):
    """Turn every view's measures into Ossie model-level metrics.
    ...
    matters). The full original measure is stashed whenever the Ossie expression
...
def _convert_measure(vname, mname, metric_name, measure, effective_base):
    ...
    sql = measure.get("sql")
    expr = None
    # Reconstructible = the exporter can rebuild this measure from the Ossie
    ...
    if agg is None and sql is not None:
        # A raw-SQL measure (the aggregate is written out in the sql). Ossie
        ...
        expr, _ = omni_sql_to_ossie(str(sql), own_view=None)
        ...
        # Best-effort ANSI so Ossie consumers still see a metric; the verbatim
    ...
    if measure.get("filters"):
        _warn(scope, "measure filters have no Ossie form; expression emitted "
...
def _measure_operand(vname, sql):
    """Translate a measure's operand sql to an Ossie reference: a same-view field
    or bare column becomes `view.name` (the qualified form Ossie metrics use);
    anything else keeps its view qualifiers and is left as-is."""
    inner, _ = omni_sql_to_ossie(sql, own_view=None)
    inner = inner.strip()
