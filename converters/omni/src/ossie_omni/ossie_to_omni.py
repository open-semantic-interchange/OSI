...
"""Convert an Ossie semantic model to Omni semantic model files.
...
# One Ossie metric aggregate call maps to a structured Omni measure.
_AGG_TO_OMNI = {
    "SUM": "sum",
    "COUNT": "count",
    "AVG": "average",
...
# `view.column` -- a dotted reference an Ossie metric uses to point into a dataset.
_DOTTED_REF_RE = re.compile(
    r"(?<![\w.$])([A-Za-z_][A-Za-z0-9_]*)\.([A-Za-z_][A-Za-z0-9_]*)(?![\w.])"
)


def convert_ossie_to_omni(ossie_yaml_str, base_view=None, dialect=None):
    """Parse Ossie YAML and return Omni model files as {relative filename: YAML str}.
...
def _convert_model(model, explicit_base_view, dialect):
    ...
    # model-wide Omni settings have no Ossie source to generate from).
    ...
    # Relationships: converted Ossie relationships in order, with any joins a
    ...
    # Measures (from Ossie metrics) attach to views.
    measures_by_view = {}
    for metric in model.get("metrics", []) or []:
        ...
        if mname.lower() in {m.lower() for m in target}:
            raise ConversionError(
                ...
                f"'{target_view}'; rename one in the Ossie model.")
    ...
    # fresh topic is only generated for hand-authored Ossie (no stash).
...
def _convert_dataset(ds, vname, view_names, dialect):
    """Build one Omni view dict from an Ossie dataset. Returns (view, dims) where
...
def _convert_field(field, dname, expr, fscope):
    ...
    if "sql" in stash:
        ...
        # other views would need ${view.field} form; Ossie field expressions are
...
def _convert_relationship(rel, view_names, imported=False):
    ...
    # Ossie orientation is many(from) -> one(to). The stash holds the declared
    # type/join_type verbatim (even an explicit Omni default). An imported join
    # with no stashed key genuinely omitted it; fresh Ossie states many_to_one.
...
def _convert_metric(metric, resolve_base, view_names, dims_by_view, dialect):
    """Map one Ossie metric to an Omni measure. Returns (view_name, measure_name,
    ...
    # stashes the original; restore it, else sanitize the Ossie name.
    ...
    if m and _balanced(m.group(2)):
        ...
        if func == "COUNT" and inner == "*":
            ...
            if dotted and dotted.group(1) in sanitized_views:
                ...
                measure = {"sql": ossie_expr_refs_to_omni(inner, sanitized_views),
                           "aggregate_type": agg}
    ...
    if measure is None:
        ...
        measure = {"sql": ossie_expr_refs_to_omni(expr, sanitized_views)}
        if len(referenced) > 1:
