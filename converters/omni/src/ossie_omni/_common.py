...
"""Shared helpers for the Ossie <-> Omni converters.
...
# Ossie `dimension.is_time` when no exact list is stashed.
...
# reader too. Same approach as the ossie-databricks converter.
...
def sanitize_name(name, what, taken):
    """Coerce an Ossie name into a valid Omni identifier.
...
def _json_default(o):
    for tag, cls in _JSON_TEMPORAL.items():
        if type(o) is cls:
            return {"__ossie_omni__": tag, "v": o.isoformat()}
    raise TypeError(f"Object of type {type(o).__name__} is not JSON serializable")


def _json_object_hook(d):
    cls = _JSON_TEMPORAL.get(d.get("__ossie_omni__", ""))
    if cls is not None and set(d) == {"__ossie_omni__", "v"}:
        return cls.fromisoformat(d["v"])
    return d


def read_stash(obj):
    """Return the OMNI stash dict on an Ossie object, or {} if absent.
...
def quote_source_part(part):
    """Quote one part of an Ossie dotted source when it needs it."""
    p = str(part)
    return p if re.fullmatch(r"[A-Za-z0-9_$]+", p) else f'"{p}"'


def parse_source(source, dataset_name):
    """Split an Ossie dataset `source` into Omni view placement.
...
def join_source(view):
    """Rebuild an Ossie dataset `source` string from an Omni view dict."""
...
def omni_sql_to_ossie(sql, own_view):
    ...
    - `${view.field[tf]}`   -> `view.field` (timeframe access has no Ossie form;
    ...
    def repl(m):
        ...
        body = m.group(1)
        if body == "TABLE":
            return "__OSSIE_TABLE__"  # handled below with its trailing dot
    ...
    out = out.replace("__OSSIE_TABLE__.", "").replace("__OSSIE_TABLE__", "")
    return out, changed
...
def ossie_expr_refs_to_omni(expr, view_names):
    """Rewrite `view.column` references in an Ossie expression to Omni `${view.column}`
