...
"""Command-line interface for the Ossie <-> Omni converter.
...
`export` converts an Ossie semantic model into an Omni model directory
(model.yaml / relationships.yaml / views/*.view.yaml / topics/*.topic.yaml);
`import` does the reverse. Import with no `-o` writes the Ossie YAML to stdout;
...
def _build_parser():
    parser = argparse.ArgumentParser(prog="ossie-omni", description=__doc__,
                                     formatter_class=argparse.RawDescriptionHelpFormatter)
    sub = parser.add_subparsers(dest="command")
    sub.required = True

    exp = sub.add_parser("export", help="Ossie semantic model -> Omni model directory")
    ...
    imp = sub.add_parser("import", help="Omni model directory -> Ossie semantic model YAML")
    ...
    imp.add_argument("--name", help="Ossie model name (default: the mapped topic's name)")
    imp.add_argument("--topic",
                     help="topic whose description/AI context map onto the Ossie model "
    ...
    try:
        if args.command == "export":
            with open(args.input) as fh:
                ossie_yaml = fh.read()
            files = convert_ossie_to_omni(ossie_yaml, base_view=args.base_view,
            ...
            out = convert_omni_to_ossie(files, model_name=args.name, topic=args.topic)
            if args.output:
