...
"""Unit tests for the Ossie -> Omni exporter."""
...
def export(ossie_yaml, **kwargs):
    with warnings.catch_warnings():
        ...
        return convert_ossie_to_omni(ossie_yaml, **kwargs)
...
def test_fixtureA_export_matches_expected():
    files = export(load_fixture("fixtureA_ossie.yaml"))
...
def test_explicit_base_view_overrides_heuristic():
    files = export(load_fixture("fixtureA_ossie.yaml"), base_view="customer")
    topic = parse(files["topics/sales.topic.yaml"])
...
def test_multiple_roots_require_base_view():
    ossie = minimal(datasets=[{"name": "a", "source": "db.s.a"},
...
def test_sanitization_collision_rejected():
    ossie = minimal(datasets=[{"name": "Orders", "source": "db.s.a"},
...
def test_unknown_relationship_dataset_rejected():
    ossie = minimal(relationships=[{"name": "r", "from": "orders", "to": "ghost",
...
def _warnings_of(ossie_yaml, **kwargs):
    with warnings.catch_warnings(record=True) as ws:
        warnings.simplefilter("always")
        convert_ossie_to_omni(ossie_yaml, **kwargs)
...
def test_field_without_usable_dialect_warns_and_drops():
    ossie = minimal(datasets=[
        {"name": "orders", "source": "db.s.t",
         "fields": [_field("x", dialect="MDX")]}])
    msgs = _warnings_of(ossie)
    assert any("no usable" in m for m in msgs)
    assert "dimensions" not in parse(export(ossie)["views/orders.view.yaml"])
