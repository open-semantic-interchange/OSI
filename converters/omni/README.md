...
# Ossie <-> Omni converter

Bidirectional, offline conversion between an Ossie semantic model and
...
this converter maps one Ossie YAML document to/from the Omni model layout:

```
model.yaml                  # model-wide config (restored verbatim from a prior import)
relationships.yaml          # top-level list of join definitions
views/<name>.view.yaml      # one per Ossie dataset
topics/<name>.topic.yaml    # one generated topic per Ossie model (or restored originals)
```
...
- **Export** (`ossie-omni export`): Ossie -> Omni files. Datasets become views,
...
- **Import** (`ossie-omni import`): Omni files -> Ossie. Omni features Ossie has no
  native field for are preserved in `custom_extensions[OMNI]`, so
  **Omni -> Ossie -> Omni is lossless**.

On **export** (Ossie -> Omni), Ossie features with no Omni slot -- `unique_keys`,
...
**import** (Omni -> Ossie), Omni-only features (formats, timeframes, hidden/tags,
...
```bash
pip install ossie-omni          # once published to PyPI
...
```bash
ossie-omni export -i model.yaml -o omni_model/ [--base-view orders] [--dialect SNOWFLAKE]
ossie-omni import -i omni_model/ [-o model.yaml] [--name my_model] [--topic orders]
```
...
Ossie YAML goes to stdout. `--base-view` picks the dataset the generated topic is
...
files = convert_ossie_to_omni(ossie_yaml_str)          # -> {relative filename: YAML str}
ossie_yaml = convert_omni_to_ossie(files)              # {relative filename: YAML str} -> str
```

## Mapping

Each row maps in both directions; the **Notes** flag where a behavior is
specific to **export** (Ossie -> Omni) or **import** (Omni -> Ossie).
...
| `custom_extensions[OMNI]` | everything Omni-only | Import stashes; export restores -- keeping `Omni -> Ossie -> Omni` lossless. |
...
non-canonical `on_sql`), joins Ossie cannot represent (non-equi/range joins,
...
raw-SQL), and files with no Ossie form (query views, extends-only views,
unrecognized files).

**Expression dialects**: Omni SQL is the SQL of the model's database
connection, and the Ossie dialect enum has no `OMNI` entry -- so import emits
...
- Exported `on_sql` references columns as `${view.column}`. Omni resolves these
  ...
  column, so the reference is valid even when the Ossie model declares no field
  for the column.
- `dimension: {is_time: false}` is equivalent to omitting `dimension` and is
  normalized away on a round trip.
- One generated topic per Ossie model; multi-path (aliased) join fan-out and
...
- Ossie metric order is regrouped by view on import (order is not semantic).
...
Both the Ossie specification and Omni's model YAML are still evolving. As either
...
topics, measure filters as first-class Ossie once the spec grows a slot for
