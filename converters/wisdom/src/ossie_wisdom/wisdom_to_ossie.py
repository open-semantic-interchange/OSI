...
_DIALECT_MAP: Dict[str, OssieDialect] = {
    "snowflake": OssieDialect.SNOWFLAKE,
    "databricks": OssieDialect.DATABRICKS,
    "bigquery": OssieDialect.BIGQUERY,
    "ansi": OssieDialect.ANSI_SQL,
    "ansi_sql": OssieDialect.ANSI_SQL,
}

_BACKTICK_DIALECTS = {OssieDialect.DATABRICKS, OssieDialect.BIGQUERY}
...
class WisdomToOssieConverter:
    ...
    def convert(self, export: dict) -> ConverterResult[OssieDocument]:
    ...
    def _resolve_dialect(self, zsheet: dict, dialect_by_connection: Dict[str, OssieDialect]) -> OssieDialect:
        connection_id = zsheet.get("location", {}).get("connectionId", "")
        return dialect_by_connection.get(connection_id, OssieDialect.ANSI_SQL)
    ...
    def _convert_table(self, zsheet: dict, dialect: OssieDialect, issues: List[ConverterIssue]) -> OssieDataset:
    ...
    def _convert_column(self, column: dict, dialect: OssieDialect) -> OssieField:
        ...
        return OssieField(
            ...
            dimension=OssieDimension(is_time=True) if properties.get("dataType") in _TIME_DATA_TYPES else None,
            label=properties.get("displayName") or None,
            description=column.get("description") or None,
        )

    def _convert_formula(self, formula: dict, dialect: OssieDialect) -> Optional[OssieField]:
    ...
    def _convert_measures(
        self, zsheet: dict, dialect: OssieDialect, issues: List[ConverterIssue]
    ) -> List[Tuple[str, OssieMetric]]:
        table_name = zsheet.get("ref", {}).get("name", "")
        metrics: List[Tuple[str, OssieMetric]] = []
    ...
    def _dedupe_metrics(
        self, metrics: List[Tuple[str, OssieMetric]], issues: List[ConverterIssue]
    ) -> List[Tuple[str, OssieMetric]]:
        result: List[Tuple[str, OssieMetric]] = []
        seen: Set[str] = set()
    ...
    def _quote_identifier(self, name: str, dialect: OssieDialect) -> str:
    ...
    def _make_expression(self, expression: str, dialect: OssieDialect) -> OssieExpression:
