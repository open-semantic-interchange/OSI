 ...
 * Uses real example file: salesforceToOssie.json
 */
class SalesforceToOssieConverterTest {
    ...
    static void checkSchemaAvailability() {
        schemaExists = SalesforceToOssieConverterTest.class
                .getResourceAsStream(SchemaValidator.SALESFORCE_SCHEMA_PATH) != null;
        ossieSchemaExists = SalesforceToOssieConverterTest.class
        ...
        if (!warningPrinted) {
            if (!schemaExists) {
                ...
                System.err.println("  Skipping SalesforceToOssieConverterTest tests.");
    ...
    void setUp() throws IOException {
        ...
        converter = ConverterFactory.getConverter(ConversionDirection.SALESFORCE_TO_OSSIE);
        yamlMapper = new ObjectMapper(new YAMLFactory());
        salesforceJson = Files.readString(Paths.get("src/test/resources/examples/salesforceToOssie.json"));
    }
    ...
    @Test
    void testOutputCompilesWithOssieSchema() throws Exception {
        List<String> results = converter.convert(salesforceJson);
        String ossieYaml = results.get(0);

        Map<String, Object> ossieRoot = yamlMapper.readValue(ossieYaml, Map.class);

        SchemaValidator validator = new SchemaValidator(new ObjectMapper(), SchemaValidator.OSSIE_SCHEMA_PATH);
        assertDoesNotThrow(() -> validator.validate(ossieRoot), "Output should comply with Ossie schema");
    }

    @Test
    void testPipelineConfigForSalesforceToOssie() {
        // Verify pipeline configuration for salesforceToOssie direction
        PipelineConfig config =
            PipelineConfigLoader.loadFromResource();

        // Verify handler list for salesforceToOssie
        List<String> handlers = config.getPipelines().get("salesforceToOssie");
        ...
        // Verify direction config for salesforceToOssie
        DirectionConfig dirConfig =
            config.getDirectionConfigs().get("salesforceToOssie");
    ...
    @Test
    void testHandlerFactoryForSalesforceToOssie() {
        // Verify HandlerFactory creates handlers for salesforceToOssie direction
        ...
        ConversionDirection direction = ConversionDirection.SALESFORCE_TO_OSSIE;
